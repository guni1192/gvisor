...
func (ndp *ndpState) doSLAAC(prefix tcpip.Subnet, pl, vl time.Duration) {
	...
	state := slaacPrefixState{
		deprecationTimer: tcpip.MakeCancellableTimer(&ndp.nic.mu, func() {
			...
			ndp.notifySLAACAddressDeprecated(tcpip.AddressWithPrefix{
...
// notifySLAACAddressDeprecated notifies the stack's NDP dispatcher that addr
// has been deprecated.
func (ndp *ndpState) notifySLAACAddressDeprecated(addr tcpip.AddressWithPrefix) {
	if ndpDisp := ndp.nic.stack.ndpDisp; ndpDisp != nil {
